...
// Floating Chat Button with Aman
function FloatingChatButton({ handleAgentClick }) {
  return (
    <button
      onClick={() => handleAgentClick({ name: 'Aman', avatar: ishaan, status: 'Available now', online: true })}
      className="fixed bottom-6 right-6 z-50 bg-blue-500 hover:bg-blue-600 text-white rounded-full shadow-lg flex items-center justify-center w-16 h-16 text-2xl animate-bounce transition-all"
    ...
    const res = await axios.post(
      GROQ_API_URL,
      {
        ...
        messages: [
          { 
            role: 'system', 
            content: `You are a helpful support agent named ${selectedAgent.name} for a clothing store called ${storeName}. 
                     Be friendly, concise, and helpful. Keep responses under 3 sentences unless more detail is requested.`
          },
          ...updatedMessages.map((msg) => ({
            role: msg.sender === 'user' ? 'user' : 'assistant',
            content: msg.text,
    ...
    <div className="bg-gray-50">
      ...
      <section className="max-w-4xl mx-auto px-4 mb-12">
        ...
        <div className="flex justify-center mt-4">
          <button
            onClick={() => handleAgentClick({ name: 'Aman', avatar: ishaan, status: 'Available now', online: true })}
            className="flex items-center gap-2 px-6 py-3 bg-blue-500 hover:bg-blue-600 text-white font-bold rounded-full shadow-lg text-lg transition"
          >
            <span className="text-xl">💬</span> Live Chat with Aman
          </button>
        </div>
      </section>
      ...
      {/* Newsletter Box */}
      <NewsLetterBox />
      <FloatingChatButton handleAgentClick={handleAgentClick} />
      {/* Customer Review Submission Section */}
      <ReviewForm />
    </div>
  );
